...
string containtment(Point p, Rectangle r) {
    string contains;
    if (p.x > r.xmin and p.x < r.xmax and p.y > r.ymin and p.y < r.ymax) contains = "inside";
    else if (p.x < r.xmin or p.x > r.xmax or p.y < r.ymin or p.y > r.ymax) contains = "outside";
    else contains = "border";
